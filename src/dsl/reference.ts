...
import { GLYPH_PRESET_NAMES, FABRIC_PRESET_NAMES, COMPOSED_PRESET_NAMES } from "./glyphs";
...
Recipe — base defense (enemies pour in from the top and advance on a base at the bottom, so a respawn never lands on the base):
  ...
  },
  {
    ...
    summary: "draw entities as a bitmap / named preset / GIF-like animation / indexed multi-colour (palette) sprite; rotate; flash on hit",
...
- "glyph": [ "<row>", ... ]  — raw rows of a small bitmap when no preset fits. A cell is "on" for any char except space, "." or "0". Drawn scaled to the entity's box in its color. Author it FACING UP. SIZE: 5x5 and 8x8 are the sweet spot — easy to keep coherent. Up to 16x16 is allowed for more detail, but the bigger the grid the easier it is to MISCOUNT a row and skew the whole sprite, so above 8x8: make every row EXACTLY the same length (a ragged row = a bug), lean on left/right symmetry, and double-check it. For anything bigger than ~16x16 or a structure, build it from "tiles" (below) instead of one giant grid. The validator WARNS (non-blocking) on a grid past 16x16 or ragged rows.
PREFER a plain single-layer glyph (no palette) — it's the most flexible (its colour is the entity colour, reusable as a tile). Add the two extras below ONLY when you need them:
- PALETTE (multi-COLOUR in ONE grid): keep "glyph" as rows but read each cell as an INDEXED colour via "palette": { "<char>": "#rrggbb", ... }. A grid char WITH an entry is drawn in that colour; a char with NO entry (any letter not in the palette) is the ENTITY colour (so it stays recolourable); "."/" "/"0" is empty. So a tank = "glyph":["Y..ZZ..Y","YXXXXXXY",...] + "palette":{"Y":"#2a2a2a","Z":"#383838"} — Y tracks, X hull (entity colour), Z barrel — one layer, a few colours. Keep it to ~6 colours or fewer (the validator WARNS above that) — a char with no entry already gives you the entity colour for free. Built-in painted presets you can use by name: ${COMPOSED_PRESET_NAMES.join(", ")} (e.g. "glyph":"pinetree"). The MATERIAL ones (brick2, pipe2, leaf2, bush2, mountain2, rock2, cloud2, drop2) are RECOLOURABLE — their body has no palette entry, so the entity/cell colour tints them while accents (mortar, snow) stay fixed.
- TILES (assemble a BIG item from small tiles): "tiles": [ [ <cell>, <cell>, ... ], ... ]  — a grid of tile cells forming one big composite sprite on a single entity. Each cell is a preset name (mono OR a painted v2; drawn in the entity color), a { "glyph": <preset or rows>, "color": "#rrggbb" } (a tile in its own colour — for a recolourable v2 like brick2 the colour tints the body, accents keep theirs), or null/"."/"" for a gap. For building structures prefer the FABRIC material tiles (single-layer, fully recolourable): ${FABRIC_PRESET_NAMES.join(", ")}. Build a castle from "stone" + "arch" + a recoloured "brick2" course, a house from "brickwork" + "shingle" + "window", etc., as ONE entity. Priority: tiles > palette > glyph/frames.
...
Recipes:
  ...
  • a PAINTED multi-colour tree (one entity, one grid + palette):  { "id":"tree","kind":"obstacle","shape":"square","color":"#2e8b3d","size":24,"control":"none","glyph":["...XX...","..XXXX..",".XXXXXX.","XXXXXXXX",".XXXXXX.","..XXXX..","...TT...","...TT..."],"palette":{"T":"#7a4a23"},"spawn":{"x":200,"y":300} }   (X = canopy in the entity colour, T = brown trunk; or just "glyph":"pinetree")
  • a BIG castle TILE assembled from small tiles (one entity; flag ON TOP, above the wall):  { "id":"castle","kind":"goal","shape":"square","color":"#9aa0aa","size":60,"control":"none","tiles":[[".",".",{"glyph":"flag","color":"#e23d3d"},".","."],["brick",".","brick",".","brick"],["brick","brick","brick","brick","brick"],["brick","brick","brick","brick","brick"],["brick","brick",".","brick","brick"]],"spawn":{"x":400,"y":300} }   (cells = preset names or {glyph,color}; null/"." = a gap)
