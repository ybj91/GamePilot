...
- LAYERS (multi-COLOUR at the same spot): "parts": [ { "glyph": <rows or preset>, "color": "#rrggbb" }, ... ]  — a stack of layers drawn back-to-front, so one entity is a little coloured sprite (a tree = brown trunk layer + green canopy layer; a face = yellow + dark features). Each part's "glyph" is inline rows or a monochrome preset name to reuse; OMIT a layer's "color" to make it inherit the ENTITY colour (that's how a layer stays recolourable). Built-in composed presets: ${COMPOSED_PRESET_NAMES.join(", ")} (e.g. "glyph": "pinetree"). The MATERIAL ones (brick2, pipe2, leaf2, bush2, mountain2, rock2, cloud2, drop2) are RECOLOURABLE — their body layer takes the entity/cell colour, so you can tint them to any palette; accents (mortar, snow) stay fixed. Use when an item needs MULTIPLE colours in the same square.
- TILES (assemble a BIG item from small tiles): "tiles": [ [ <cell>, <cell>, ... ], ... ]  — a grid of tile cells forming one big composite sprite on a single entity. Each cell is a preset name (mono OR composed v2; drawn in the entity color), a { "glyph": <preset or rows>, "color": "#rrggbb" } (a tile in its own colour — for a recolourable v2 like brick2 the colour tints the body, accents keep theirs), or null/"."/"" for a gap. For building structures prefer the FABRIC material tiles (single-layer, fully recolourable): ${FABRIC_PRESET_NAMES.join(", ")}. Build a castle from "stone" + "arch" + a recoloured "brick2" course, a house from "brickwork" + "shingle" + "window", etc., as ONE entity. Priority: tiles > parts > glyph/frames.
