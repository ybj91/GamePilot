...
- **Composition — use only when needed** (a plain single-layer glyph is the flexible default — you control its colour and can reuse it as a tile):
  - **`palette`** (multi-colour in *one* indexed grid): keep `glyph` as rows, add `palette: { "<char>": "#rrggbb", … }` so each grid cell is drawn in its mapped colour. A char with **no** palette entry renders in the **entity** colour (so it stays recolourable); `"."`/`" "`/`"0"` is empty. One entity becomes a little coloured sprite (a tank = `Y` tracks + `X` hull-in-entity-colour + `Z` barrel). Keep it to **~6 colours or fewer** (the validator warns above that) — a char with no entry already gives you the entity colour for free. Built-in painted presets (usable by name): `pinetree`, `cottage`, `daisy`, `toadstool`, + colour remakes `tank2`/`heart2`/`star2`/`coin2`/`face2`/`hero2`/… The material ones (`brick2`/`pipe2`/`leaf2`/`bush2`/`mountain2`/`rock2`/`cloud2`/`drop2`) are recolourable (no-palette-entry body).
  - **`tiles`** (*assemble a big item* from small tiles): a 2D grid `[[<cell>,…],…]` forming one big composite sprite on a single entity. Each cell is a preset name (mono or painted; entity color), a `{ glyph, color }` (a tile in its own colour), or `null`/`"."`/`""` for a gap — e.g. a castle from `stone`/`arch`/`brick2` tiles. Priority: `tiles` > `palette` > `glyph`/`frames`.
