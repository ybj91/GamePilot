...
- **Composition — use only when needed** (a plain single-layer glyph is the flexible default — you control its colour and can reuse it as a part/tile):
  - **`parts`** (multi-colour *layers* at the same spot): `[{ glyph: <rows|preset>, color? }, …]` drawn back-to-front — one entity becomes a little coloured sprite (a tree = brown-trunk + green-canopy layer; a face = yellow + dark features). `color` defaults to the entity color. Built-in composed presets: `pinetree`, `cottage`, `daisy`, `toadstool`, + colour remakes `tank2`/`heart2`/`star2`/`coin2`/`face2`/`hero2`.
  - **`tiles`** (*assemble a big item* from small tiles): a 2D grid `[[<cell>,…],…]` forming one big composite sprite on a single entity. Each cell is a preset name (entity color), a `{ glyph, color }` (a tile in its own colour), or `null`/`"."`/`""` for a gap — e.g. a castle from `brick`/`door` tiles. Priority: `tiles` > `parts` > `glyph`/`frames`.
