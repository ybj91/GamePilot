...
- **`glyph`**: a **preset name** (a built-in common shape — `tank`, `ship`, `arrow`, `heart`, `star`, `diamond`, `plus`, `ring`, `face`, `skull`, `alien`, a platformer set `hero`/`coin`/`brick`/`flag`/`mushroom`/`pipe`, plus the self-animating `invader`, `blob`, `flame`, `explosion`, `goomba`) *or* raw rows of a small bitmap (a cell is "on" for any char but space/`.`/`0`). Drawn scaled in the entity's color instead of the bare shape. Still no assets — just data.
