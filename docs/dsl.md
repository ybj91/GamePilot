...
- **`parts`** (composition, multi-colour): a stack of layers `[{ glyph: <rows|preset>, color? }, …]` drawn back-to-front — one entity becomes a little *coloured sprite* (a tree = brown-trunk layer + green-canopy layer). Each part's `glyph` is inline rows (any size, 8×8 recommended for detail) or a monochrome preset name to reuse; `color` defaults to the entity color. Built-in composed presets: `pinetree`, `cottage`, `daisy`, `toadstool` (use via `glyph:"pinetree"`). Overrides `glyph`/`frames`.
