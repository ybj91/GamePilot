...
- `world.ts` — mutable game state: live entity instances, score, **global `vars`** (named counters from `spec.vars`, accessed via `getVar`/`setVar`; `score` is unified into these), status, spawn/respawn/`maintain` logic, `nearestOf` (aim targeting), and `stepLifetimes` (decrements the reserved `ttl` prop and reaps projectiles). **Entity types created only via rules (bullets) must set `spawn.count: 0`** or they pre-spawn at start.
